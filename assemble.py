...
# - Matches each satellite image of a cyclone with the maximum sustained wind speed of that cyclone
...
# The number of pixels wide and tall to crop the images of cyclone to
side_length = 50

# Lists to hold the cyclone images and the wind speed associated with those images. These lists are aligned so that
...
for i in range(len(files)):
    ...
    # Get storm name, date, and time of the cyclone from the image's file name
    file_name = files[i]
    ...
    # Filter the best track dataset to find the row that matches the name, date, and time of this cyclone image
    ...
    # Get the wind speed from the row that matches the name, date, and time of this cyclone image
    try:
        ...
        continue  # Skip to the next cyclone image if the a wind speed could not be found for this cyclone image

    # Add the image and wind speed to these lists. This way, the lists of images and labels always line up. The first
    # cyclone image in the images list is associated with the first wind speed in the labels list.
